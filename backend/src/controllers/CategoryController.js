...
export class CategoryController {
	 ...
	 * @param {object} req - Express request object. Expects 'name', 'image' and 'allowedAttributes' in req.body.
	 * @param {object} res - Express response object.
	 * @returns {Promise<void>} - Responds with status 201 and the created CategoryDTO.
	 */
	create = async (req, res) => {
		const { allowedAttributes, ...rest } = req.body;
	...
	getAll = async (req, res) => {
		...
		const categories = await this.#categoryService.getAll(page, limit, { ...rest });

		return res.status(200).json(categories);
	}

	/**
	 * Retrieves a single category's full details by its Slug, delegating the fetch operation to the service layer. (Public endpoint).
	 * @param {object} req - Express request object. Expects 'slug' in req.params.
	 * @param {object} res - Express response object.
	 * @returns {Promise<void>} - Responds with status 200 and the requested CategoryDTO.
	 */
	getBySlug = async (req, res) => {
		const { slug } = req.params;
		const result = await this.#categoryService.getBySlugOrFail(slug);
